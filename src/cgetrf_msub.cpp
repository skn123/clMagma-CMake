/*
    -- clMAGMA (version 1.1.0) --
       ...
       @generated from zgetrf_msub.cpp normal z -> c, Fri Jan 10 15:51:17 2014
    ...
    if (nb <= 1 || nb >= n) {
        ...
        for (i=0; i<tot_subs; i++) {
            ...
            if (trans == MagmaNoTrans) {
                ...
                if ((m == n_local[i]) && (m%32 == 0) && (ldda%32 == 0) && (ldda == lddat)) {
                    d_lAT[i] = d_lA[i];
                    magma_ctranspose_inplace( d_lA[i], 0, ldda, ldda, queues[2*(i%num_gpus)+1] );
                } else {
                    #ifdef SINGLE_GPU_PER_CONTEXT
        ...
        for (d=0; d<tot_subs; d++) {
            if (trans == MagmaNoTrans) {
                ...
                if (d_lAT[d] == d_lA[d]) {
                    magma_ctranspose_inplace( d_lA[d], 0, ldda, ldda, queues[2*(d%num_gpus)+1] );
                } else {
